    ...
    const codeAction = async () => {
      ...
      try {
        ...
        const templateContent = fs
          .readFileSync(
            path.join(process.env.LEETCODE_TEMPLATE_PATH, codeTemplateFileName),
          )
          .toString();
...
function addContestProgram(program: Command) {
  ...
  const getDefaultContestSlug = () => {
    return process.env.INIT_CWD?.split(path.sep).slice(-2).join("-") as string;
  };
    ...
    const buildAction = async () => {
      ...
      try {
        ...
        if (fs.existsSync(navTemplateFile)) {
          navObject = Yaml.load(fs.readFileSync(navTemplateFile, "utf8"));
        } else {
          navObject = { nav: [] };
        }
        ...
        const buildContent = async (name: string) => {
          ...
          const problemSrcPath = path.join(leetcodeSrc, docsRelativePath);
